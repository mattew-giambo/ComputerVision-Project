...
The detection of synthetically generated images has been an active research area since the rise of GANs. Early work by Wang *et al.* (2020) demonstrated that a simple ResNet-50 trained on ProGAN images generalises surprisingly well to other GAN architectures when augmented with blur and JPEG compression during training. This finding established the importance of input-level data augmentation as a proxy for real-world post-processing degradations.

Subsequent work has leveraged **frequency-domain features** as a more robust signal. It has been observed that GAN-generated images exhibit characteristic spectral artefacts — such as periodic peaks in the Fourier magnitude spectrum — that arise from the convolutional upsampling operations inherent in generator architectures. Methods such as those by Frank *et al.* (2020) exploit these spectral fingerprints directly, computing the 2D DFT of images and using the resulting magnitude spectra as classification inputs.

More recently, the emergence of diffusion models (e.g., DALL-E, Stable Diffusion, Midjourney) has challenged existing detectors, as diffusion-based images do not exhibit the same spectral artefacts as GAN images. This has prompted the community to develop more general features. Patch-based approaches, self-supervised pre-training, and Vision Transformer (ViT) backbones have all been explored as means of capturing richer, less architecture-specific signals.
...
One line of research addresses this through explicit augmentation strategies — training detectors on images that have been subjected to simulated JPEG compression and noise — while another seeks architecturally robust representations. The use of **noise-sensitive filters** (e.g., high-pass filters applied before the first convolutional layer, as in Fridrich and Kodovsky's SRM features) has proven effective in extracting residual signals that survive many post-processing operations.
...
The integration of **depth information** (RGB-D learning) has been explored extensively in autonomous driving, scene understanding, and 3D object detection. In the context of forensics, depth maps offer a complementary signal: AI-generated images may exhibit depth-consistency artefacts invisible in the RGB domain, such as implausible perspective transitions or geometrically inconsistent shadow-depth relationships. Monocular Depth Estimation (MDE) models — particularly lightweight transformer-based architectures such as Depth Anything — enable the offline extraction of proxy depth maps from monocular RGB images, making RGB-D forensics tractable without depth sensors.
